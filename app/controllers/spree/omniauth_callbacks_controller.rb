      ...
      class_eval %Q{
        ...
        def #{provider}
          if request.env["omniauth.error"].present?
            flash[:error] = Spree.t("devise.omniauth_callbacks.failure", :kind => auth_hash['provider'], :reason => Spree.t(:user_was_not_valid))
          ...
          if authentication.present?
            flash[:notice] = Spree.t("devise.omniauth_callbacks.success", :kind => auth_hash['provider'])
            sign_in_and_redirect :spree_user, authentication.user
          elsif spree_current_user
            spree_current_user.apply_omniauth(auth_hash)
            spree_current_user.save!
            flash[:notice] = Spree.t("devise.sessions.signed_in")
            redirect_back_or_default(account_url)
          else
            user = Spree::User.find_by_email(auth_hash['info']['email']) || Spree::User.new
            user.apply_omniauth(auth_hash)
            if user.save
              flash[:notice] = Spree.t("devise.registrations.signed_up")
